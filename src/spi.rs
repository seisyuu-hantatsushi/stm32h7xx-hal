                ...
                impl Spi<$SPIX, Enabled, $TY> {
                        ...
                        CONFIG: Into<Config>,
                    {
                        ...
                        spi.cr2().write(|w| unsafe {
                            w.tsize().bits(matches!(config.hardware_cs.mode,
                                                    HardwareCSMode::FrameTransaction) as u16)
                        });
            ...
            impl<WORD> Spi<$SPIX, Enabled, WORD> {
                ...
                pub fn kernel_clk(clocks: &CoreClocks) -> Option<Hertz> {
                    ...
                    match ccip1r.spi123sel().variant() {
                        Some(ccip1r::SPI123SEL::Pll1Q) => clocks.pll1_q_ck(),
                        Some(ccip1r::SPI123SEL::Pll2P) => clocks.pll2_p_ck(),
                        Some(ccip1r::SPI123SEL::Pll3P) => clocks.pll3_p_ck(),
                        // Need a method of specifying pin clock
                        Some(ccip1r::SPI123SEL::I2sCkin) => unimplemented!(),
                        Some(ccip1r::SPI123SEL::Per) => clocks.per_ck(),
                        _ => unreachable!(),
                    }
                }
                /// Returns the frequency of the current kernel clock
