...
impl SystemWindowWatchdog {
    ...
    pub fn new(wwdg: WWDG, ccdr: &Ccdr) -> Self {
        ...
        ccdr.rcc.apb3enr().modify(|_, w| w.wwdg1en().set_bit());
        #[cfg(feature = "rm0455")]
        ccdr.rcc.apb3enr().modify(|_, w| w.wwdgen().set_bit());
...
impl Watchdog for SystemWindowWatchdog {
    ...
    fn feed(&mut self) {
        // if this value is 0 it is assumed that the watchdog has not yet been started
        assert!(self.down_counter != 0);
        self.wwdg.cr().modify(|_, w| unsafe { w.t().bits(self.down_counter) });
    }
}
...
impl WatchdogEnable for SystemWindowWatchdog {
        ...
        T: Into<Self::Time>,
    {
        ...
        self.wwdg.cfr().modify(|_, w| unsafe { w.wdgtb().bits(wdgtb) });
        self.wwdg.cfr().modify(|_, w| unsafe { w.w().bits(self.down_counter) });
        self.wwdg.cr().modify(|_, w| unsafe { w.t().bits(self.down_counter) });
