            ...
            impl GetClk for $TIMX {
                ...
                fn get_clk(clocks: &CoreClocks) -> Option<Hertz> {
                    ...
                    match d3ccipr.read().lptim345sel().variant() {
                        Some(srdccipr::LPTIM345SEL::RccPclk4) => Some(clocks.pclk4()),
                        Some(srdccipr::LPTIM345SEL::Pll2P) => clocks.pll2_p_ck(),
                        Some(srdccipr::LPTIM345SEL::Pll3R) => clocks.pll3_r_ck(),
                        Some(srdccipr::LPTIM345SEL::Lse) => unimplemented!(),
                        Some(srdccipr::LPTIM345SEL::Lsi) => clocks.lsi_ck(),
                        Some(srdccipr::LPTIM345SEL::Per) => clocks.per_ck(),
                        _ => unreachable!(),
                    }
                }
            }
